...
const menuItems = [
  ...
  },
  {
    ...
    subItems: [
      ...
      { name: 'Commission', path: '/admin/reports/commissions' }
    ]
  },
  {
...
const AdminSidebar = () => {
  const pathname = useClientPathname('/admin');
  ...
  return (
    <div className="w-64 bg-gray-800 text-white h-screen fixed left-0 top-0 overflow-y-auto">
      ...
      <nav className="mt-6">
        {menuItems.map((item) => {
          const isExpanded =
            expandedItems[item.path] || currentPath.startsWith(item.path);
          const isActive = currentPath.startsWith(item.path);

          return (
          <div key={item.path} className="mb-1">
            <div
              className={`flex items-center justify-between px-4 py-3 cursor-pointer ${isActive ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
              onClick={() => {
              ...
              <div className="bg-gray-900 pl-12">
                {item.subItems.map((subItem) => (
                  <Link href={subItem.path} key={subItem.path} passHref>
                    <div
                      className={`block px-4 py-2 cursor-pointer ${currentPath === subItem.path ? 'text-blue-300' : 'hover:text-blue-200'}`}
                    >
                      {subItem.name}
                    </div>
                  </Link>
